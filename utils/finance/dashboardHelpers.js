...
export function normalizeTransactions(items = [], options = {}) {
	...
	const activeFamilyId = Number(options?.familyId || 0) > 0 ? Number(options.familyId) : null;
	...
	return base.map((item) => ({
		...
		family_id:
			(item?.family_id ?? null) || (Number(item?.is_family_shared ?? 0) === 1 ? activeFamilyId : null),
		referenceDate: resolveTransactionDate(item),
	}));
}

export function applyVisibilityScopeFilter(items = [], scope = "all", familyId = null, userId = null) {
	const base = Array.isArray(items) ? items : [];
	const numericFamilyId = Number(familyId || 0);

	return base.filter((item) => {
		const effectiveFamilyId =
			(item?.family_id ?? null) || (Number(item?.is_family_shared ?? 0) === 1 && numericFamilyId > 0 ? numericFamilyId : null);
...
export function buildMonthlySeries(items = []) {
	...
	const groupedKeys = buildMonthKeyList(monthKeys[0], monthKeys[monthKeys.length - 1]);
...
export function buildCategoryExpenses(items = [], top = 6) {
	const base = normalizeTransactions(items).filter((item) => item.tipo === "pagar");
...
export function applyRangeFilter(items = [], range = "90d") {
	...
	return base.filter((item) => item.referenceDate && item.referenceDate >= fromKey);
}
