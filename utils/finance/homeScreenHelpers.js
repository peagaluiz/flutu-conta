...
export function filterLancamentosByDateRange(lancamentos = [], startDate, endDate) {
  ...
  return base.filter((item) => {
    const referenceDate = item?.data_transacao ? String(item.data_transacao).slice(0, 10) : "";
...
export function getLatestLancamento(lancamentos = []) {
  ...
  return base.reduce((latest, item) => {
    ...
    return Number(item?.id_transacao || 0) > Number(latest?.id_transacao || 0) ? item : latest;
  }, null);
}

export function buildQuickActions({ ultimaCategoria, ultimoTipo, onOpenInsert }) {
  return [
    {
      key: "despesa",
      title: "Despesa",
    ...
    },
    {
      ...
      title: "Categoria",
      subtitle: ultimaCategoria || "Usar a categoria do último lançamento",
      icon: Clock3,
      className: "bg-[#334155] border-[#1e293b]",
      onPress: () => onOpenInsert({ tipo: ultimoTipo, categoria: ultimaCategoria || undefined }),
    },
  ];
}

export function filterLancamentosByRange(lancamentos = [], filterType = "all", referenceDate = new Date()) {
  const base = Array.isArray(lancamentos) ? lancamentos : [];
  ...
  if (filterType === "today") {
    ...
    return base.filter((item) => {
      const ref = item?.data_transacao ? String(item.data_transacao).slice(0, 10) : "";
      return ref === todayIso;
    });
  }
  ...
  if (filterType === "month") {
    ...
    return base.filter((item) => String(item?.data_transacao || "").startsWith(prefix));
  }
...
export function getFilterLabel(filterType) {
  return TRANSACTION_FILTER_OPTIONS.find((item) => item.value === filterType)?.label || "Todos";
}
