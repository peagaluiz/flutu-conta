...
import { getFilterLabel, TRANSACTION_FILTER_OPTIONS } from "@/utils/finance/homeScreenHelpers";
    ...
    >
      <Box className="flex-row items-center justify-between">
        <HStack space="sm" className="items-center">
          ...
          <Text className="font-semibold" style={{ color: colors.textPrimary }}>Últimos lançamentos</Text>
        </HStack>
      ...
      {loading ? (
        ...
        <Box className="gap-1">
          {items.slice(0, previewLimit).map((item) => (
            <LancamentoListItem key={String(item.id_transacao)} item={item} onPress={() => onPressItem(item)} />
          ))}

          {items.length > previewLimit ? (
            <Pressable onPress={onPressSeeAll}>
              <Box className="flex-row items-center justify-center gap-2 rounded-lg border px-3 py-3" style={{ backgroundColor: colors.surfaceMuted, borderColor: colors.border }}>
      ...
      <Actionsheet isOpen={isFilterSheetOpen} onClose={onCloseFilterSheet}>
        <ActionsheetBackdrop />
        <ActionsheetContent>
          <ActionsheetDragIndicatorWrapper>
            <ActionsheetDragIndicator />
          ...
          {TRANSACTION_FILTER_OPTIONS.map((option, index) => (
            <ActionsheetItem
              key={option.value}
              style={{ marginBottom: index === TRANSACTION_FILTER_OPTIONS.length - 1 ? 35 : 5 }}
              onPress={() => {
