        ...
        <CardBase colors={colors} danger={pending}>
            <VStack className="gap-1">
                <Text className="font-semibold" style={{ color: colors.textPrimary }}>
                    <Text style={{ color: colors.textSecondary }}>{item.id_pessoa ? `#${item.id_pessoa} ` : ""}</Text>
                    {item.nome || "Sem nome"}
                </Text>

                <Text className="text-xs" style={{ color: colors.textSecondary }}>
                    {pending ? `Pendente • ${Number(item.pending_count || 0)} lançamento(s)` : "Registro simples"}
                </Text>
            </VStack>
            ...
            <HStack className="mt-3 gap-2">
                ...
                {pending ? (
                    <ActionButton label="Sincronizar" icon={RefreshCw} onPress={onSync} colors={colors} />
                ) : (
