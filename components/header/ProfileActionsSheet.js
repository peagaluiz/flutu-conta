...
export function ProfileActionsSheet({
	...
	const avatarFallback =
		...
		profileEmail?.trim()?.[0]?.toUpperCase() ||
		"U";
	...
	return (
		<Actionsheet isOpen={isOpen} onClose={onClose}>
			<ActionsheetBackdrop />
			<ActionsheetContent
				className="items-stretch px-4 pt-3"
					...
					<HStack className="items-start gap-3">
						...
						>
							<Box className="relative">
								<Avatar
									size="lg"
									className="border-2 border-outline-100"
								>
									<AvatarFallbackText>
										{avatarFallback}
									</AvatarFallbackText>
								...
								<Box className="absolute bottom-0 right-0 h-5 w-5 items-center justify-center rounded-full bg-secondary-500">
						...
						<VStack className="flex-1 items-start">
							<HStack className="w-full items-center justify-between gap-3">
								<Text
									size="lg"
									bold
									className="flex-1 text-left text-typography-900"
								>
							...
							{profileEmail ? (
								<Text
									size="sm"
									className="mt-1 text-left text-typography-500"
								>
									{profileEmail}
								</Text>
							) : null}
