...
export default function RecuperarSenhaScreen() {
    ...
    const onSubmit = async ({ email }) => {
        ...
        try {
            await withAbort(requestPasswordReset(email), requestAbortRef.current.signal);

            if (!mountedRef.current) return;

            showNewToast("success", PASSWORD_RESET_SUCCESS_MESSAGE, "Verifique seu e-mail");
            setCooldownLeft(COOLDOWN_SECONDS);
        } catch (error) {
            if (!mountedRef.current || isAbortError(error)) return;

            const genericError = "Nao foi possivel iniciar a recuperacao agora. Tente novamente.";
                ...
                >
                        <VStack space={4}>
                            <VStack className="mx-6 mt-6" space={2}>
                                <Heading size="lg" fontWeight="600" style={{ color: colors.textPrimary }}>Recuperar senha</Heading>
                            ...
                            <VStack className="mx-6" space={3}>
                                ...
                                >
                                    <ButtonText style={{ color: "#F8FAFC" }}>{sendButtonLabel}</ButtonText>
                                </Button>
                                ...
                                >
                                    <ButtonText style={{ color: colors.textPrimary }}>Voltar</ButtonText>
                                </Button>
                            </VStack>
                        </VStack>
                </Box>
