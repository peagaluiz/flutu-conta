...
export default function ViewScreen() {
  ...
  const transacaoId = useMemo(() => {
    const raw = Array.isArray(params?.id_transacao) ? params?.id_transacao[0] : params?.id_transacao;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const transacao = await database.getTransacao(transacaoId, { fallbackRemoteOnMiss: true });
  ...
  return (
    <ScrollView style={{ backgroundColor: colors.screen }} contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      {!item ? (
        <Box className="rounded-2xl border p-4" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
          <Text style={{ color: colors.textSecondary }}>Lançamento não encontrado.</Text>
        </Box>
      ) : (
        <Box className="rounded-2xl border p-4" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
          ...
          <Text style={{ color: colors.textSecondary }}>Vencimento</Text>
          <Text className="mb-2 font-semibold" style={{ color: colors.textPrimary }}>
          ...
          <Text className="font-semibold" style={{ color: colors.textPrimary }}>
            {Number(item?.is_from_recurrence || 0) === 1
              ? `Recorrência (${item?.recurrence_frequency || "mensal"})`
              : "Lançamento avulso"}
          </Text>
        </Box>
      )}
