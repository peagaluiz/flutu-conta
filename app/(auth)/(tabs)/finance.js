...
export default function Finance() {
    ...
    const filteredItems = useMemo(() => applyRangeFilter(scopedItems, range), [scopedItems, range]);
    const resumo = useMemo(() => finalizeResumo(buildResumoGeral(filteredItems)), [filteredItems]);
    const monthlySeries = useMemo(() => buildMonthlySeries(filteredItems), [filteredItems]);
    const categorySeries = useMemo(() => buildCategoryExpenses(filteredItems, 6), [filteredItems]);
    ...
    }, [activeFamilyId, canUseFamilyScope, database, userData?.id, visibilityScope]);
    ...
    return (
        <>
                ...
                <Box className="gap-4 px-3 pt-3" style={{ minHeight: "100%" }}>
                    <Box className="rounded-xl border p-3" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
                        ...
                        <HStack className="items-center justify-between">
                            ...
                            <HStack className="gap-2">
                                {VISIBILITY_OPTIONS.filter((option) => (canUseFamilyScope ? true : option.value !== "family")).map((option) => {
                    ...
                    {loading ? (
                        ...
                        <>
                            <FinanceResumoSection
                                resumo={resumo}
                                colors={colors}
                                onPressSaldo={() => openDetails("Saldo consolidado", filteredItems)}
                                onPressEntradas={() =>
                                    openDetails(
                                        "Entradas",
                                ...
                                onPressPendentes={() =>
                                    openDetails(
                                        "Pendentes",
                                        filteredItems.filter((item) => item.status !== "pago")
                                    )
                                }
                            />
                            <FinanceMonthlyChartSection
                                ...
                                onPressMonth={(monthKey) =>
                                    openDetails(
                                        ...
                                        filteredItems.filter((item) => String(item.referenceDate || "").startsWith(monthKey))
                                    )
                                }
                            />
                                ...
                                onPressCategory={(categoria) =>
                                    openDetails(
                                        ...
                                        filteredItems.filter(
                                            (item) => item.tipo === "pagar" && String(item.categoria || "") === String(categoria)
                                        )
                                    )
                                }
                            />
            ...
            <Actionsheet isOpen={detailSheetOpen} onClose={() => setDetailSheetOpen(false)}>
                <ActionsheetBackdrop />
                <ActionsheetContent style={{ paddingBottom: Math.max(insets.bottom, 12) + 8 }}>
                    <ActionsheetDragIndicatorWrapper>
                        <ActionsheetDragIndicator />
                    </ActionsheetDragIndicatorWrapper>
                    ...
                    {detailItems.length === 0 ? (
                        <ActionsheetItem isDisabled>
                            <ActionsheetItemText>Sem itens para exibir.</ActionsheetItemText>
                        </ActionsheetItem>
                    ) : (
                            ...
                            <ActionsheetItem key={`detail-${item.id_transacao}`}>
                                <Box className="w-full">
                                    <HStack className="items-center justify-between">
                                        ...
                                        <Text style={{ color: item.tipo === "receber" ? "#16A34A" : "#DC2626" }}>
                                            {formatCurrency(Number(item.valor || 0))}
                                        </Text>
                                    </HStack>
                                    <Text className="text-xs" style={{ color: colors.textSecondary }}>
                                        {item.pessoa || "Sem pessoa"} • {item.referenceDate || "sem data"}
                                    </Text>
                                </Box>
                            </ActionsheetItem>
                        ))
