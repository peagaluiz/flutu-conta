...
import { isAbortError, withAbort } from "@/services/auth/abortable";
...
export default function NovaSenhaScreen() {
    ...
    const activateRecoverySession = useCallback(
        async (url, signal) => {
            try {
                ...
                showNewToast("error", "Link de recuperacao invalido ou expirado.");
            }
        },
    ...
    useEffect(() => {
        ...
        async function prepareRecovery() {
            ...
            try {
                const initialUrl = await withAbort(Linking.getInitialURL(), prepareAbortRef.current.signal);

                if (initialUrl) {
                    await activateRecoverySession(initialUrl, prepareAbortRef.current.signal);
                } else {
                    const { data } = await withAbort(supabase.auth.getSession(), prepareAbortRef.current.signal);
    ...
    const saveLabel = useMemo(() => (isSaving ? "Salvando..." : "Salvar nova senha"), [isSaving]);
    ...
    const onSubmit = async ({ senha }) => {
        ...
        try {
            ...
            await withAbort(supabase.auth.signOut(), saveAbortRef.current.signal);

            if (!mountedRef.current) return;

            showNewToast("success", "Senha atualizada com sucesso. Faca login novamente.", "Sucesso");
            router.replace("/login");
        } catch (error) {
            if (!mountedRef.current || isAbortError(error)) return;

            const networkError = "Sem conexao com a internet. Verifique sua rede e tente novamente.";
            const genericError = "Nao foi possivel atualizar a senha agora. Tente novamente.";
            showNewToast("error", isLikelyNetworkError(error) ? networkError : genericError);
        } finally {
            ...
            <Box className="flex-1 justify-end" style={{ backgroundColor: colors.screen }}>
                ...
                >
                    <VStack space={5} className="mx-2">
                        <VStack space={2}>
                            <Heading size="lg" fontWeight="600" style={{ color: colors.textPrimary }}>Definir nova senha</Heading>
                        ...
                        {isPreparing ? null : !isLinkValid ? (
                            <VStack space={4}>
                                <Text size="sm" style={{ color: colors.dangerText }}>
                                    O link nao esta mais valido. Solicite um novo e-mail de recuperacao.
                                </Text>
                                <Button
                                    action="primary"
                                    size="lg"
                                ...
                                >
                                    <ButtonText style={{ color: "#F8FAFC" }}>Solicitar novo link</ButtonText>
                                </Button>
                            </VStack>
                        ) : (
                            <>
                                ...
                                <Button
                                    ...
                                    isDisabled={isSaving}
                                >
                                    <ButtonText style={{ color: "#F8FAFC" }}>{saveLabel}</ButtonText>
                                </Button>
                            </>
                        )}
                        ...
                        <Button
                            ...
                            isDisabled={isSaving}
                        >
                            <ButtonText style={{ color: colors.textPrimary }}>Voltar para login</ButtonText>
                        </Button>
                    </VStack>
                </Box>
            </Box>
