...
export async function initializeSQLite(database: SQLiteDatabase) {
	...
	await database.execAsync(`
		CREATE TABLE IF NOT EXISTS tipo_imobilizado (
			id_tipo_imobilizado INTEGER PRIMARY KEY,
			remote_id INTEGER,
			nome TEXT NOT NULL,
			data_sync TEXT,
			sync_status TEXT DEFAULT 'pending',
	...
	await database.execAsync(`
		CREATE TABLE IF NOT EXISTS imobilizado (
			id_imobilizado INTEGER PRIMARY KEY,
			remote_id INTEGER,
	...
	await database.execAsync("CREATE INDEX IF NOT EXISTS idx_recorrencia_transacoes_recurrencia ON recorrencia_transacoes(id_recurrencia);");

	// Migrações simples para bases já existentes
