...
export type PessoaWithPending = PessoaRow & {
	is_pending?: boolean;
...
export function createPessoaRepository() {
	return {
		...
		syncPessoaPendente: async (
			...
			const { whereClause, whereArgs } = buildPendingTransacoesScope(cleanName, pendingIds);
		...
		createPessoa: async (nome: string, options?: OwnershipOptions) => {
		...
		updatePessoaAndRelinkTransacoes: async (
			id_pessoa: number,
			previousName: string,
			nextName: string
		) => {
        ...
        `,
			...
			const scope = buildPendingTransacoesScope(cleanPreviousName, pendingIds);
			await updateTransacoesPessoaLink({
